...
function createSubscription (seneca, { client, service, options }) {
  ...
  return utils.resolve_pins(options)
    .then((pins) => {
      ...
      var tsub = options.topics.subscription
      ...
      function create_topic (topic) {
        return function (callback) {
          service.createTopicIfNotExists(topic, callback)
        }
      }
...
function createActor (seneca, { client, receiver, queue, options }, done) {
  var listener = Listener(seneca, { client, receiver, queue, options })
...
function setup (seneca, { client, service, options }, done) {
  return createSubscription(seneca, { client, service, options })
    .then(function ({ receiver, queue }) {
      return createActor(seneca, { client, receiver, queue, options }, done)
    })
}
