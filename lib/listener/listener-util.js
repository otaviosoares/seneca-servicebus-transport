...
function resolveListenQueue (pins, { prefix = '', separator = '.' } = {}) {
  ...
  var composition = groupByKeys(pins)
...
function resolveSubscriptionName (id = '', separator = '.', prefix = '') {
  var sid = id || shortid.generate()
...
function groupByKeys (pins) {
  return _.reduce(pins, _groupByKey, {})
}
