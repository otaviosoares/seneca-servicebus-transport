...
var _senders = []
function createListener (seneca, { client, receiver, queue, options }) {
  var utils = seneca.export('transport/utils')

  function handleMessage (message, replyWith, receiver) {
    var data = utils.parseJSON(seneca, `listen-${options.type}`, message)
